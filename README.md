...
* Check the `ConnectionStrings` in `appsettings.json` files under the `TestProject.Web` and `TestProject.DbMigrator` projects and change it if you need.
...
```bash
dotnet dev-certs https -v -ep openiddict.pfx -p fe7934b8-7c87-44c5-a9a5-edaab681c8b5
```

> `fe7934b8-7c87-44c5-a9a5-edaab681c8b5` is the password of the certificate, you can change it to any password you want.
...
Run `TestProject.DbMigrator` to create the initial database. This should be done in the first run. It is also needed if a new database migration is added to the solution later.

### Solution structure

This is a layered monolith application that consists of the following applications:

* `TestProject.DbMigrator`: A console application which applies the migrations and also seeds the initial data. It is useful on development as well as on production environment.
* `TestProject.Web`: ASP.NET Core MVC / Razor Pages application that is the essential web application of the solution.
